...
def ensure_state():
    if "profiles_df" not in st.session_state:
        # Start with a small sample for speed; adjustable in sidebar
...
def log_interaction(viewer_key: str, viewer_name: str, profile_row: pd.Series, action: str, compatibility: float):
    """Append a single interaction row to a CSV on disk."""
...
st.caption("Switch the viewer to see the site from any user's perspective — and log every interaction to CSV.")
    ...
    with mid:
        ...
        if st.button("➕ Add Viewer") and new_name:
            if new_name in st.session_state.users:
                ...
                st.rerun()
    with right:
        # New: Create viewer from an existing profile row (view-as-profile)
        ...
        if st.button("👤 Use this profile as viewer"):
            ...
            # Build viewer settings from the profile
            ...
            st.session_state.users[vname] = {
                "settings": {
                    ...
                    "age": int(pr["age"]),
                    "city": pr.get("city", ""),
                    # Safe default: show all genders; you can customize later
                    "seeking": GENDERS[:],
...
# --- Sidebar: per-user settings + dataset controls + logging path ---
with st.sidebar:
    st.header("Your Settings (for this viewer)")
    ustate = get_active()
    s = ustate["settings"]
    # Build a dynamic city list from current dataset for better UX
    ...
    # interests universe from generator
    ...
    if st.button("🔁 Regenerate dataset"):
        ...
        # Reset browse index for each viewer
...
with tabs[3]:
    ...
    st.info(
        ...
        "All interactions are now appended to the CSV you chose in the sidebar."
    )
